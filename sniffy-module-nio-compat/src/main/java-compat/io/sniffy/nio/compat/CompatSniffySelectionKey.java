...
public class CompatSniffySelectionKey extends SelectionKey implements ObjectWrapper<SelectionKey> {
    ...
    protected CompatSniffySelectionKey(CompatSniffySelector sniffySelector, SelectableChannel sniffyChannel, Object attachment) {
        if (null != attachment) {
            attach(attachment);
        }
        ...
        LOG.trace("Created new CompatSniffySelectionKey(" + delegate + ", " + sniffySelector + ", " + sniffyChannel + ") = " + this);
    }

    public void setDelegate(SelectionKey delegate) {
        if (null != this.delegate) {
            if (!AssertUtil.logAndThrowException(LOG, "Trying to set CompatSniffySelectionKey.delegate twice", new IllegalStateException())) {
                LOG.info("Trying to set CompatSniffySelectionKey.delegate twice");
            }
        } else {
    ...
    public Selector selector() {
        ...
        if (!isValid() &&
            ...
            return NoOpSelector.INSTANCE; // TODO: make CompatNoOpSelector
        } else {
            return sniffySelector;
        }
        */
