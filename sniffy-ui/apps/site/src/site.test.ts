...
describe('@sniffy/site workspace contract', () => {
  ...
  it('keeps Java-resource production builds separate from delegated site commands', () => {
    ...
    expect(workspacePackageJson.scripts).toMatchObject({
      ...
      'test:site':
        'npm run build:site && vitest run --project unit apps/site/src/site.test.ts apps/site/src/source-snippets.test.ts apps/site/src/product-version.test.ts apps/site/src/documentation.test.ts apps/site/src/legacy-docs.test.ts && npm test --workspace @sniffy/site',
