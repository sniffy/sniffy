...
describe('@sniffy/site workspace contract', () => {
  ...
  it('keeps Java-resource production builds separate from delegated site commands', () => {
    ...
    expect(workspacePackageJson.scripts).toMatchObject({
      ...
      'test:site':
        'npm run build:site && npm run verify:site-search-index && vitest run --project unit apps/site/src/site.test.ts apps/site/src/homepage.test.ts apps/site/src/source-snippets.test.ts apps/site/src/product-version.test.ts apps/site/src/documentation.test.ts apps/site/src/deterministic-search-index.test.ts apps/site/src/theme/SearchBar/index.test.tsx apps/site/src/theme/SearchBar/search-index.test.ts && npm test --workspace @sniffy/site',
