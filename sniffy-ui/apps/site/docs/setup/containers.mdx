...
In order to do that unzip the `sniffy-@sniffy.version@-jboss-module.zip` (available in maven Nexus central repository as `io.sniffy:sniffy:jar:jboss-module` artifact) archive to `modules/system/layers/base` folder.
...
<SourceSnippet file="sniffy/src/jboss-module/module.xml" region="WildFlyModule" language="xml" />
...
If you're using datasources managed within Tomcat rather that within your web application, place `sniffy-@sniffy.version@.jar` to the `lib` folder
