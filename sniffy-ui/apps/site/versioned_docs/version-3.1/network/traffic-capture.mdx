...
Sniffy provides an alternative implementation of `SSLEngine`, `SSLSocketFactory` and other classes from JSSE framework. Since some of these constructs can be cached in application code, please make sure to initialize Sniffy as early as possible. If absolutely necessary you can initialize Sniffy as a javaagent, although it’s a bit cumbersome - see [Standalone Setup](/docs/3.1/installation/) section for details.

Sniffy support custom JSSE providers such as BouncyCastle. However if you’re installing **AFTER** you initialize Sniffy you _might_ need to reinitialize Sniffy. It’s possible using `Sniffy.reinitialize()` method. In general it will work out of the box without any further actions from developer given that Sniffy is initialized on an early stage.
