...
|monitorNio
|Monitor IP TCP `SocketChannel` and `ServerSocketChannel` connections. UNIX-domain channels and UDP are pass-through; NIO2/AIO is unsupported. See <<nio-monitoring,NIO monitoring>>.
|false; enable explicitly with this setting, `monitor-nio`, or the agent parameter
