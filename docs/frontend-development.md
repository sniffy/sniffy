...
The path-filtered `Validate website` workflow handles site, shared-theme, and site-input changes independently from the Maven release pipeline. It reports lint, formatting, typecheck, site contract tests, the Docusaurus production build, and desktop/mobile Playwright smoke coverage as separate steps, then uploads the complete `apps/site/build/` directory as a downloadable artifact. Use the manual workflow dispatch when a validation run is needed without a matching path change; it never deploys the site.
