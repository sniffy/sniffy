...
Dependency security follows [`docs/dependency-security.md`](dependency-security.md). Run
`npm run dependency-security:test` for the deterministic offline comparison contracts and
`npm run dependency-security:validate` for exception/override registry checks. Pull requests that change npm inputs
compare clean base and proposed-merge audits under the same Node/npm toolchain and current advisory data. Pull requests
with an unchanged graph report an explicit skip instead of failing on unrelated advisory drift. The independent
`Dependency Security` workflow reports the complete current repository audit on a weekly schedule and after dependency
inputs land on the default branches.
