...
- Keep `gh` installation, persistent authentication, token scope/organization approval, write-enabled agent network policy, and reversible publication validation documented and tested.
- Use a dedicated agent account and repository-scoped credential where possible. Protect `develop` with server-side rules and no token-owner bypass.
- Treat the persisted credential as readable by agent-phase processes. Never put it in remote URLs or logs; revoke it and reset the Cloud cache after any suspected exposure.
- Reset the Cloud cache after setup-script, secret, or environment-policy changes.

The separate `SNIFFY_GITHUB_PAT` experiment in PR #635 was useful for identifying the secret-lifetime, identity, least-privilege, and ruleset risks. Its parallel bootstrap is not adopted: the current `GH_TOKEN` plus verified GitHub CLI credential-helper path is the single supported mechanism.
