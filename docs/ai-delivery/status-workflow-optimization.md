...
credentials even though each job needs only a bounded file set. This change narrows checkout without changing the established job
runners, Project queries, artifact schema, permissions, or scheduling semantics.
...
The validation job requires the workflow, normalizer and mergeability helper, focused tests, policy tests, profile, prompts, and
documentation checked by those tests. The publication job requires only `.github/scripts/delivery-status.js` and
`.github/scripts/delivery-status-mergeability.js` from trusted `develop`.
...
setup, GitHub API latency, git object negotiation, or artifact upload time. The established runner behavior remains independently
covered by the runner policy test.
