...
branch_input="$(mktemp)"
profile_input="$(mktemp)"
trap 'rm -f "$branch_input" "$profile_input"' EXIT
...
jq -n --arg body "$body" --arg defaultProfile "$DEFAULT_WORKER_PROFILE" \
  '{body:$body, defaultProfile:$defaultProfile}' >"$profile_input"
profile_json="$(node "$SCRIPT_DIR/cli-worker-state.js" profile "$profile_input")"
worker_profile="$(jq -r .profile <<<"$profile_json")"
profile_source="$(jq -r .source <<<"$profile_json")"
case "$worker_profile" in
  economy)
    model="$ECONOMY_MODEL"
    reasoning="$ECONOMY_REASONING"
    ;;
  balanced)
  ...
  frontier)
    ...
    ;;
  *)
    echo "Unsupported Codex worker profile: $worker_profile" >&2
    exit 1
    ;;
esac
[[ "$model" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{2,127}$ ]] || { echo "Invalid configured Codex model." >&2; exit 1; }
case "$reasoning" in low|medium|high|xhigh) ;; *) echo "Invalid configured reasoning effort." >&2; exit 1 ;; esac
...
spawning_reference="v=2;state=spawning;adapter=codex-cli-v1;generation=${generation};token=${token};branch=${branch};profile=${worker_profile};model=${model};reasoning=${reasoning};leaseUntil=${provisional_lease}"
jq -n \
  ...
  --arg workerProfile "$worker_profile" --arg profileSource "$profile_source" \
  ...
  '{generation:$generation,token:$token,type:$type,number:$number,status:$status,mode:$mode,branch:$branch,
    branchSource:$branchSource,baseSha:$baseSha,workerProfile:$workerProfile,profileSource:$profileSource,
...
Codex worker profile: ${worker_profile} (${profile_source})
...
while (( SECONDS < deadline )); do
  if [[ -s "$generation_path/ack.json" ]]; then
    ...
    if wait_control_reaction "$update_comment"; then
      ...
      printf '{"outcome":"DISPATCHED","generation":"%s","thread":"%s","branch":"%s","profile":"%s","model":"%s","reasoning":"%s"}\n' \
        "$generation" "$thread_id" "$branch" "$worker_profile" "$model" "$reasoning"
      exit 0
    fi
